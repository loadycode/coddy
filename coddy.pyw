...
filetypes=[("all filetypes","*"),
	...
	("c++ (.cpp)","*.cpp"),
	("c/c++ headers (.h)","*.h")];
edited=False;openfile=None
# one of keyboard keys pressed
def editevent(event):
...
default_bg="#444444";default_fg="white";default_panel="silver";default_cursor="orange";default_font="Consolas 10";default_side="silver";default_side_fg="black";default_panel_fg="black";separator="-";icon="src/ico/logo.png";sideshow="False";panelshow="True"
...
def mapped(event):
	...
	window.call("wm","iconphoto",window._w,iconimg)
	window.iconphoto(True,iconimg)
	if isMapped==1:set_appwindow(window)
	isMapped=0
isMapped=0;isMaximised=False
window=tk.Tk();window.title("coddy")
window.bind("<Map>",mapped)
window.overrideredirect(True)
screenwidth=window.winfo_screenwidth()
screenheight=window.winfo_screenheight()
xcoor=(screenwidth/2)-(600/2);ycoor=(screenheight/2)-(400/2);window.minsize(width=600,height=450)
...
tile=tk.Frame(window,height=40,bg="white");tile.bind("<B1-Motion>",moving);tile.bind("<Button-1>",clickpos);tile.pack(side="top",fill="x");iconimg=tk.PhotoImage(file=icon);icon=tk.Button(tile,image=iconimg,relief="flat",width=23,height=23,borderwidth=0);icon.pack(side="left");tilelabel=tk.Label(tile,text="coddy",justify="center",bg="white",font="Consolas 10");tilelabel.bind("<B1-Motion>",moving);tilelabel.bind("<Button-1>",clickpos);tilelabel.pack(side="left");tileclose=tk.Frame(tile,height=25,width=25,bg="red");tileclose.bind("<Button-1>",close);tileclose.pack(side="right");tilemax=tk.Frame(tile,height=25,width=25,bg="green");tilemax.bind("<Button-1>",maximise);tilemax.pack(side="right");tilemin=tk.Frame(tile,height=25,width=25,bg="blue");tilemin.bind("<Button-1>",minimise);tilemin.pack(side="right");side=tk.Label(window,bg="white",fg=default_side_fg,anchor="n",font=default_font);side.pack(side="left",fill="y")
...
checkconf();textbox.insert("1.0",open("src/launch").read());minimise(1);window.geometry("{}x{}+{}+{}".format(600,400,int(xcoor),int(ycoor)));mapped(1);window.mainloop()